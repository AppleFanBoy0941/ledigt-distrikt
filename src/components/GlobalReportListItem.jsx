...
export default function GlobalReportListItem({ report }) {
	...
	return (
		<li className='pt-6 px-6 pb-2 bg-white rounded-3xl shadow-2xl shadow-slate-400/20 overflow-hidden'>
			<header className='mb-1 flex items-center justify-between'>
				<h3 className='font-header text-xl font-extrabold text-slate-500'>
...
function Item({ type, count, bg }) {
	return (
		<motion.li
			...
			viewport={{ once: true }}
			className={`-mx-4 p-4 py-2 rounded-2xl flex justify-between items-center ${bg && 'bg-slate-50/75'}`}
		>
